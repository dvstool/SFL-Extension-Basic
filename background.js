...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  if (message.type === 'SHOW_READY_NOTIFICATION') {
    showReadyNotification(message.message || 'A scanned timer is ready.', message.id || `sfl-ready-${Date.now()}`).then(sendResponse);
    return true;
  }
...
async function showReadyNotification(message, id, iconUrl = notificationIcon) {
  try {
    ...
    console.error('Unable to create Sunflower Tools notification:', error);
