  ...
  const update = () => {
    document.querySelectorAll('[data-countdown-target]').forEach((element) => {
      ...
      if (card.dataset.resource === 'composter') promoteGrowingComposter(card);
        ...
        miningRefreshTimer = window.setTimeout(() => void scanMap('mining'), 700);
        ...
        fruitRefreshTimer = window.setTimeout(() => void scanMap('fruit', { forceTimerRefresh: true }), 700);
        ...
        card.querySelector('.crop-card-meta').textContent = '?ang c?p nh?t Salt?';
        element.remove();
        window.setTimeout(() => void scanMap('salt'), 700);
      }
