...
(() => {
  ...
  const english = {
    ...
    upgradePlan: 'Upgrade plan', upgradeDescription: 'Enter a Gold or Diamond key to replace the current key.',
  ...
  const pairs = [
    ...
    ['N\u00e2ng c\u1ea5p g\u00f3i', 'Upgrade plan'], ['Nh\u1eadp key Gold ho\u1eb7c Diamond \u0111\u1ec3 thay th\u1ebf key hi\u1ec7n t\u1ea1i.', 'Enter a Gold or Diamond key to replace the current key.'],
  ...
  Object.entries({
    ...
    Activity: 'Hoạt động', Foraging: 'Tìm kiếm', Home: 'Trang chủ', Logs: 'Nhật ký', Settings: 'Cài đặt',
    ...
    'Load Tools': 'Quét Tools', Buy: 'Mua', 'Basic User': 'Người dùng Basic', 'Gold User': 'Người dùng Gold', 'Diamond User': 'Người dùng Diamond', 'Free plan': 'Gói miễn phí', 'Expires: Unlimited': 'Hạn dùng: Không giới hạn', 'Expires: Unknown': 'Hạn dùng: Không xác định', 'License expired': 'Key đã hết hạn', 'Checking expiry...': 'Đang kiểm tra hạn dùng...',
  ...
  const translateValue = (value, language) => {
    ...
    if (language === 'en') {
      return value
        .replace(/^Thu ho\u1ea1ch x(\d+)$/, 'Collect x$1').replace(/^Tr\u1ed3ng x(\d+)$/, 'Plant x$1')
    ...
    return value
      .replace(/^Collect x(\d+)$/, 'Thu ho\u1ea1ch x$1').replace(/^Plant x(\d+)$/, 'Tr\u1ed3ng x$1')
